...
from dataset import create_NSD_dataset
...
def wandb_init(config, output_path):
    wandb.init( project='SD_finetune',
                group="stageB_dc-ldm",
                anonymous="allow",
                config=config,
                reinit=True)
...
def main(config):
    ...
    if config.dataset == 'NSD':
        fmri_latents_dataset_train, fmri_latents_dataset_test = create_NSD_dataset(config.nsd_path, config.patch_size,
                fmri_transform=identity, image_transform=[img_transform_train, img_transform_test],
                )
    ...
    pretrain_sphericalmae_path = config.pretrain_sphericalmae_path
    # create generateive model
    generative_model = fLDM(pretrain_sphericalmae_path,device=device, pretrain_root=config.pretrain_gm_path, logger=config.logger,
    ...
    if config.checkpoint_path is not None:
        ...
        print('model resumed')
    # finetune the model
    trainer = create_trainer(config.num_epoch, config.precision, config.accumulate_grad, logger, check_val_every_n_epoch=10)
