...
def wandb_init(config):
    wandb.init( project="SD-finetune",
                group='eval',
                anonymous="allow",
                config=config,
                reinit=True)
...
if __name__ == '__main__':
    ...
    if target == 'NSD':
        _, dataset_test = create_NSD_dataset(config.nsd_path, config.patch_size,
    ...
    pretrain_sphericalmae_metafile = config.pretrain_sphericalmae_path
    # create generateive model

    generative_model = fLDM(pretrain_sphericalmae_metafile, device=device, pretrain_root=config.pretrain_gm_path,
    ...
    grid, samples = generative_model.generate(dataset_test, config.num_samples, 
                ...
                vqgan_model=None) # generate 10 instances
